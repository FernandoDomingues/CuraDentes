// ═══════════════════════════════════════════════════════════════════════════════
// SUGESTÕES DE ENDEREÇO — Autocomplete fuzzy de endereços.
//
// Fornece sugestões de endereço com correção fuzzy enquanto o usuário digita.
// Port do hook useAddressSuggestions do site-k11 para o R0 (Next.js).
//
// Estratégia:
//   1. Busca uma vez do Supabase (cliente público/anônimo) todas as cidades,
//      bairros e nomes de clínica dos endereços cadastrados (campos leves).
...
async function fetchLocations(): Promise<RawLocation[]> {
  ...
  try {
    ...
    console.warn("[sugestoes] Falha ao ler cache de endereços:", err);
  }

  // Busca do Supabase (cliente público/anônimo)
  ...
  try {
    ...
    console.warn("[sugestoes] Falha ao salvar cache de endereços:", err);
  }
...
export function useAddressSuggestions(query: string, maxResults = 6): {
  ...
  useEffect(() => {
    ...
    fetchLocations()
      ...
      .catch((err) => console.error("[sugestoes] Erro ao carregar locais:", err))
