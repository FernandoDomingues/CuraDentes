...
//
// Port fiel do site-k11 (src/lib/dentistCache.ts) para o R0.
// Responsabilidades:
...
//   4. Cache por query (buildQueryCacheKey/saveQueryCache/loadQueryCache) para
//      restaurar resultados instantaneamente ao dar F5.
//
// SSR-safe: todo acesso a localStorage é protegido por `typeof window`.
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 * Isso permite que a página de perfil carregue instantaneamente (0ms) usando os
 * dados em cache enquanto atualiza os detalhes em background.
 */
  ...
  try {
    ...
    dentists.forEach((newD) => {
      ...
      if (idx >= 0) {
        ...
        currentResults[idx] = {
          ...currentResults[idx],
          ...Object.fromEntries(Object.entries(mapped).filter(([, v]) => v !== "" && (Array.isArray(v) ? v.length > 0 : true)))
        };
      } else {
