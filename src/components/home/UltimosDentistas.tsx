...
// ═══════════════════════════════════════════════════════════════════════════════
// SEÇÃO: UltimosDentistas — Últimos dentistas cadastrados (expansível)
//
// Mostra na home os 15 dentistas mais recentes da plataforma. O cabeçalho é um
// botão que expande/recolhe um carrossel (mobile) / grade (desktop) de cards.
// Cada card leva ao perfil público (/dentista/{cro|id}) com foto, nome, CRO,
// selo de verificação, bio e chips de especialidades.
//
// Dados: busca os pros recentes em `curadentespro` e, para cada um, agrega as
// atividades dos seus endereços (`curadentespro_enderecos`) num Set (sem dupes).
// Cache opcional em localStorage por 1 minuto para evitar refazer a query.
//
// Portado do k11 (LatestDentists). Precisa de "use client" por usar useState/
// useEffect + fetch no Supabase e navegação por clique.
// ═══════════════════════════════════════════════════════════════════════════════
...
// Caminho público do logo alternativo (fallback de foto).
...
export default function UltimosDentistas() {
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1200px]">
        ...
        {expandido && (
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-3 mt-6 animate-in slide-in-from-top-4 fade-in duration-300">
            ...
            <div
              key={dentista.id}
              onClick={() => router.push(`/dentista/${dentista.cro || dentista.id}`)}
